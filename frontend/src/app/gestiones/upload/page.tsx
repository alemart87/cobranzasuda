...
export default function GestionesUploadPage() {
    ...
    const interval = setInterval(async () => {
      try {
        const data = await apiFetch<{ status: string; last_error: string | null }>(
          `/api/v1/gestiones/uploads/${uploadId}`,
        );
        setStatus(data.status);
        if (data.status === "completed") {
          clearInterval(interval);
          const reports = await apiFetch<{ items: { id: string }[] }>("/api/v1/gestiones/reports");
          if (reports.items[0]) router.push(`/gestiones/reports/${reports.items[0].id}`);
  ...
  const onSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!file) { setError("Seleccionar archivo"); return; }
    setError(null);
    setSubmitting(true);
    setStatus("uploading");
    ...
    try {
      ...
      const r = await fetch("/api/v1/gestiones/uploads", {
        method: "POST",
        body: form,
    ...
    <AppShell>
      <div className="mb-6">
        ...
        <p className="text-sm text-brand-slate mt-1">
          Archivo Excel exportado del CRM con columnas: <code>Usuario</code>, <code>Estado</code>, <code>Subestado</code>, <code>Lead - Asegurado</code>, etc.
        </p>
      </div>
