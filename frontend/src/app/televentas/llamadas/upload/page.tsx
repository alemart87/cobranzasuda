...
export default function TeleventasLlamadasUploadPage() {
    ...
    const interval = setInterval(async () => {
      try {
        const data = await apiFetch<{ status: string; last_error: string | null }>(
          `/api/v1/televentas/llamadas/uploads/${uploadId}`,
        );
        setStatus(data.status);
        if (data.status === "completed") {
          clearInterval(interval);
          const reports = await apiFetch<{ items: { id: string }[] }>("/api/v1/televentas/llamadas/reports");
          if (reports.items[0]) router.push(`/televentas/llamadas/reports/${reports.items[0].id}`);
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const r = await fetch("/api/v1/televentas/llamadas/uploads", {
    ...
    <AppShell>
      ...
      <div className="mb-6">
        ...
        <p className="text-sm text-brand-slate mt-1">
          Export de voz saliente (Usuarios, Fecha, Duración…). Acepta <code>.xlsx</code> y <code>.xls</code>.
        </p>
      </div>

      <form onSubmit={onSubmit} className="card p-7 max-w-3xl space-y-5">
        <div>
          <label className="label">Archivo de Llamadas</label>
