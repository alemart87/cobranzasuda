...
export default function TeleventasProduccionUploadPage() {
    ...
    const interval = setInterval(async () => {
      try {
        const data = await apiFetch<{ status: string; last_error: string | null }>(
          `/api/v1/televentas/produccion/uploads/${uploadId}`,
        );
        setStatus(data.status);
        if (data.status === "completed") {
          clearInterval(interval);
          const reports = await apiFetch<{ items: { id: string }[] }>("/api/v1/televentas/produccion/reports");
          if (reports.items[0]) router.push(`/televentas/produccion/reports/${reports.items[0].id}`);
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const r = await fetch("/api/v1/televentas/produccion/uploads", {
  ...
  return (
    <AppShell>
      <div className="mb-2 text-xs text-brand-slate">
        ...
        <span className="text-brand-ink font-semibold">Subir Producción</span>
      </div>
      <div className="mb-6">
        <h1 className="font-display text-3xl text-brand-ink uppercase">Subir Libro de Producción</h1>
        <p className="text-sm text-brand-slate mt-1">
          Detalle de ventas de pólizas (Prima, Producto, Nombre Referencia…). Emisiones y anulaciones.
      ...
      <form onSubmit={onSubmit} className="card p-7 max-w-3xl space-y-5">
        <div>
          <label className="label">Libro de Producción</label>
