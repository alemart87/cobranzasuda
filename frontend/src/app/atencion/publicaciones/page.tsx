...
type Kind = "atencion_llamadas" | "atencion_gestiones";
...
const KIND_META: Record<Kind, { label: string; api: string; view: (id: string) => string; badge: string }> = {
  atencion_llamadas: {
    ...
    api: "/api/v1/atencion/llamadas/reports",
    view: (id) => `/atencion/llamadas/reports/${id}`,
    badge: "bg-brand-cyan/10 text-brand-cyan",
  },
  atencion_gestiones: {
    label: "Reporte de Gestiones",
    api: "/api/v1/atencion/gestiones/reports",
    view: (id) => `/atencion/gestiones/reports/${id}`,
...
const KIND_FILTERS = [
  ...
  { value: "atencion_llamadas", label: "Llamadas" },
  { value: "atencion_gestiones", label: "Gestiones" },
];
...
export default function AtencionPublicacionesPage() {
  ...
  const load = async () => {
    try {
      ...
      const merged: Publication[] = [
        ...ll.items.map((r) => ({ kind: "atencion_llamadas" as Kind, id: r.id, title: r.title, period_month: r.period_month, generated_at: r.generated_at, is_published: r.is_published })),
        ...ge.items.map((r) => ({ kind: "atencion_gestiones" as Kind, id: r.id, title: r.title, period_month: r.period_month, generated_at: r.generated_at, is_published: r.is_published })),
      ];
  ...
  const togglePublish = async (p: Publication) => {
    ...
    setActionError(null);
    try {
      await apiFetch(`${KIND_META[p.kind].api}/${p.id}/publish`, {
        method: "POST",
  ...
  return (
    <AppShell>
      ...
      <div className="mb-6">
        <h1 className="font-display text-3xl sm:text-4xl text-brand-ink uppercase">Publicaciones · Atención</h1>
        <p className="text-sm text-brand-slate mt-1 max-w-3xl">
          Publicar, despublicar y eliminar los reportes de Atención al Cliente (llamadas y gestiones).
          Los clientes solo ven lo que está publicado.
        </p>
      </div>

      {error && (
        <div className="card p-4 mb-4"><p className="text-brand-primary text-sm">{error}</p></div>
      )}
      {actionError && (
        <div className="mb-4 flex items-start justify-between gap-3 rounded-md border border-brand-primary/30 bg-brand-primary-light px-4 py-3">
          ...
          <button onClick={() => setActionError(null)} className="text-brand-primary-dark/60 hover:text-brand-primary-dark text-lg leading-none" aria-label="Cerrar">×</button>
        </div>
      )}
              ...
              {filtered.map((p) => (
                <tr key={`${p.kind}-${p.id}`} className="border-t border-brand-border hover:bg-brand-bg-soft">
                  <td className="px-4 py-3">
                    <span className={`text-[10px] uppercase tracking-wider2 font-semibold px-2 py-1 rounded ${KIND_META[p.kind].badge}`}>
                  ...
                  <td className="px-4 py-3">
                    {p.is_published ? <span className="badge-success">Publicado</span> : <span className="badge-neutral">Borrador</span>}
                  </td>
                  ...
                  <td className="px-4 py-3 text-right whitespace-nowrap">
                    ...
                    <button onClick={() => setPendingDelete(p)} disabled={busyId === p.id} className="text-xs px-2.5 py-1 mr-1 rounded border border-brand-border hover:border-brand-primary hover:text-brand-primary disabled:opacity-50">
                      Eliminar
                    </button>
          ...
          pendingDelete && (
            <>
              ...
              <b className="text-brand-ink">
                {KIND_META[pendingDelete.kind].label} · {pendingDelete.title || pendingDelete.period_month || "sin período"}
              </b>
