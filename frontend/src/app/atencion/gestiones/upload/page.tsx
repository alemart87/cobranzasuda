...
export default function AtencionGestionesUploadPage() {
    ...
    const interval = setInterval(async () => {
      try {
        const data = await apiFetch<{ status: string; last_error: string | null }>(
          `/api/v1/atencion/gestiones/uploads/${uploadId}`,
        );
        setStatus(data.status);
        if (data.status === "completed") {
          clearInterval(interval);
          const reports = await apiFetch<{ items: { id: string }[] }>("/api/v1/atencion/gestiones/reports");
          if (reports.items[0]) router.push(`/atencion/gestiones/reports/${reports.items[0].id}`);
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const r = await fetch("/api/v1/atencion/gestiones/uploads", {
        method: "POST",
        body: form,
  ...
  return (
    <AppShell>
      <div className="mb-2 text-xs text-brand-slate">
        ...
        <span className="text-brand-ink font-semibold">Subir Gestiones</span>
      </div>
      <div className="mb-6">
        <h1 className="font-display text-3xl text-brand-ink uppercase">Subir Reporte de Gestiones</h1>
        <p className="text-sm text-brand-slate mt-1">
          Export del CRM de atención. Acepta <code>.xlsx</code> y <code>.xls</code>.
        </p>
      </div>

      <form onSubmit={onSubmit} className="card p-7 max-w-3xl space-y-5">
        <div>
          <label className="label">Archivo de Gestiones</label>
          <input
            type="file"
            accept=".xlsx,.xls"
            onChange={(e) => setFile(e.target.files?.[0] ?? null)}
            className="block w-full text-sm text-brand-slate file:mr-4 file:py-2.5 file:px-5 file:rounded-md file:border-0 file:text-xs file:font-semibold file:uppercase file:tracking-wider2 file:bg-brand-ink file:text-white hover:file:bg-brand-purple cursor-pointer"
          />
          {file && (
