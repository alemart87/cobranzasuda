...
export function NotasSimulacion({ notas, setNotas, nombres, guardaSola }: {
  ...
  return (
    <section className="card p-5 border-l-4 border-brand-ink">
      <div className="flex flex-wrap items-baseline justify-between gap-2 mb-1">
        ...
        <span className="text-[11px] text-brand-slate">
          {notas.length} nota(s){guardaSola ? " · se guardan solas con la simulación abierta" : notas.length ? " · se guardan al guardar la simulación" : ""}
        </span>
      </div>
      <p className="text-xs text-brand-slate mb-3 max-w-3xl">
        El registro razonado del trabajo: qué se asumió, qué se ve, qué se decidió, qué queda pendiente y qué puede salir distinto.
        Cada nota lleva autor y fecha, y puede referirse a un mes en particular. Se imprimen con el informe.
      </p>

      {/* ===== Alta ===== */}
      <div className="no-print rounded-md border border-brand-border bg-brand-bg-soft p-3 mb-4">
        <textarea value={texto} onChange={(e) => setTexto(e.target.value)} rows={2}
          placeholder='Ej.: "Se asume zafra promedio 2025; si la retención del mes 1 baja del 80%, el resultado a 18 meses se da vuelta."'
          className="input w-full !py-1.5 text-sm" />
        <div className="flex flex-wrap items-center gap-2 mt-2">
          <div className="flex flex-wrap gap-1">
            {Object.entries(TIPOS_NOTA).map(([k, t]) => (
              <button key={k} onClick={() => setTipo(k)} title={t.ayuda}
                className={`px-2 py-0.5 rounded-full border text-[11px] font-bold ${t.cls} ${tipo === k ? "ring-2 ring-brand-ink/40" : "opacity-60 hover:opacity-100"}`}>
                {t.label}
              </button>
            ))}
          </div>
          <select value={mes} onChange={(e) => setMes(e.target.value)} className="text-[11px] border border-brand-border rounded px-2 py-1 bg-white">
            <option value="">Sobre toda la simulación</option>
            {nombres.map((nm, i) => <option key={i} value={i + 1}>Sobre: {nm || `Mes ${i + 1}`}</option>)}
          </select>
          <button onClick={agregar} disabled={!texto.trim()} className="btn-primary !py-1 !px-4 text-sm disabled:opacity-50 ml-auto">Agregar nota</button>
        </div>
      </div>
      ...
      {notas.length === 0 ? (
        ...
        <ol className="space-y-2">
          {visibles.map(({ n, i }) => {
            ...
            return (
              <li key={n.id ?? `i-${i}`} className="rounded-md border border-brand-border bg-white px-3 py-2 flex gap-3" style={{ borderLeft: `4px solid ${t.punto}` }}>
