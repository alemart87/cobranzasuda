...
async def test_registro_de_simulaciones():
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as ac:
        ...
        assert (await ac.get("/api/v1/facturacion/simulaciones", headers=h)).json()["simulaciones"] == []

        # guardar
        r = await ac.post("/api/v1/facturacion/simulaciones", headers=h, json={
        ...
        lst = (await ac.get("/api/v1/facturacion/simulaciones", headers=h)).json()["simulaciones"]
        assert [x["id"] for x in lst] == [sid] and "parametros" not in lst[0]
        det = (await ac.get(f"/api/v1/facturacion/simulaciones/{sid}", headers=h)).json()
