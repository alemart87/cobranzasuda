"""Reporte de Gestiones CRM (Televentas) — KPIs + data JSON."""
...
class TeleventasCrmReport(Base):
    __tablename__ = "televentas_crm_reports"
    ...
    contactos: Mapped[int] = mapped_column(Integer, default=0)          # con respuesta (≠ no contesta)
    aceptas: Mapped[int] = mapped_column(Integer, default=0)
    agendados: Mapped[int] = mapped_column(Integer, default=0)
    no_acepta: Mapped[int] = mapped_column(Integer, default=0)
    tasa_contacto_pct: Mapped[float] = mapped_column(Numeric(6, 2), default=0)
