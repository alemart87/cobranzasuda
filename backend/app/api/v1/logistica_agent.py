...
from ...services.agent.logistica_agent import stream_logistica_agent
from ...services.agent.pricing import compute_cost_usd
from ...services.agent.tools import AgentContext
from ...services.audit_service import record_action
from ..deps import CurrentUser, client_ip, require_logistica_access


router = APIRouter(prefix="/logistica-agent", tags=["logistica-agent"])
...
@router.get("/access")
async def access(user: CurrentUser = Depends(require_logistica_access)) -> dict:
    return {
        "enabled": True,
...
async def post_message(
    ...
    async def event_stream():
        ...
        async def _produce():
            try:
                async for ev in stream_logistica_agent(messages, context):
                ...
                if exc.__class__.__name__ == "MaxTurnsExceeded":
                    ...
                    logger.exception(f"[logistica-agent] error en {conv_id}: {exc}")
